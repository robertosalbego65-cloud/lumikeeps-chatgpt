...
function createLumiKeepsServer() {
  ...
  server.registerTool(
    "find_lumikeeps_portrait",
    {
      ...
      description:
        "Find the best LumiKeeps personalized portraits for a customer's request across the live Etsy catalog. Use for family, couples, best friends, pets, memorials, weddings, anniversaries, birthdays, Christmas, separate photos, combined photos and personalized gift ideas. After choosing the relevant results, call render_lumikeeps_portraits with their listing_ids to show the visual carousel.",
      inputSchema: {
        query: z
          .string()
          .min(2)
        ...
        content: [
          {
            ...
            text:
              ...
              `${summary}\n\nUse render_lumikeeps_portraits with these listing IDs to show the visual cards: ${results.map((item) => item.listing_id).join(", ")}`,
          },
        ],
        structuredContent: {
          query,
  ...
  server.registerTool(
    "render_lumikeeps_portraits",
    {
      title: "Show LumiKeeps Portrait Cards",
      description:
        "Render the final LumiKeeps portrait recommendations as visual cards with Etsy images and links. Always call find_lumikeeps_portrait first, then pass 1 to 3 listing_ids from its results in best-to-worst order.",
      inputSchema: {
        listing_ids: z
          .array(z.string().min(1))
          .min(1)
          .max(3)
          .describe("Listing IDs selected from find_lumikeeps_portrait, ordered best match first."),
        headline: z
          .string()
          .max(80)
          .optional()
          .describe("Short user-facing title for the card carousel."),
      },
      annotations: {
        readOnlyHint: true,
        destructiveHint: false,
      ...
      _meta: {
        ...
        "openai/toolInvocation/invoking": "Preparo le anteprime…",
        "openai/toolInvocation/invoked": "Anteprime pronte.",
      },
    },
    async ({ listing_ids, headline }) => {
      const selected = getListingsByIds(listing_ids).map(compactListing);
        ...
        content: [
          {
            ...
            text: selected.length
              ? `Showing ${selected.length} LumiKeeps portrait recommendation${selected.length === 1 ? "" : "s"}.`
              : "No matching LumiKeeps listings were found for the supplied IDs.",
          },
        ],
        structuredContent: {
          headline: headline || "Ritratti LumiKeeps consigliati",
          catalog_source: catalogSource,
          results: selected,
        },
      };
...
const httpServer = createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/") {
    res
      ...
      .end(
        `LumiKeeps MCP server V3 UI — ${listings.length} listings loaded — source: ${catalogSource}`
      );
...
httpServer.listen(port, () => {
  console.log(
    `LumiKeeps MCP server V3 UI listening on port ${port}`
  );
